...
calc_landing_adjustment_vect <- function(dat, Use_Gust) {
  ...
  if (Use_Gust) {
    ...
    dat_0_10_11_12 <- dat %>% filter(landing_stabilisation_speed_type %in% c(0, 10, 11, 12))
    dat_1_2_3_4 <- dat %>% filter(landing_stabilisation_speed_type %in% c(1, 2, 3, 4))
    dat_5 <- dat %>% filter(landing_stabilisation_speed_type == 5)
    dat_6 <- dat %>% filter(landing_stabilisation_speed_type == 6)
    dat_7 <- dat %>% filter(landing_stabilisation_speed_type == 7)
    dat_8 <- dat %>% filter(landing_stabilisation_speed_type == 8)
    dat_9 <- dat %>% filter(landing_stabilisation_speed_type == 9)
    ...
    dat_0_10_11_12 <- mutate(dat_0_10_11_12,
                             landing_adjustment = ifelse((Surface_Headwind + Gust)/2 < 5, 5, ifelse((Surface_Headwind + Gust)/2 > 20, 20, (Surface_Headwind + Gust)/2)))
    dat_1_2_3_4 <- mutate(dat_1_2_3_4,
                            landing_adjustment = ifelse(Surface_Headwind/3 < 5, 5, ifelse(Surface_Headwind/3 > 15, 15, Surface_Headwind/3)))
    dat_5 <- mutate(dat_5,
                    landing_adjustment = ifelse((Surface_Headwind + Gust)/3 < 0, 0, ifelse((Surface_Headwind + Gust)/3 > 20, 20, (Surface_Headwind + Gust)/3)))
    dat_6 <- mutate(dat_6,
                    landing_adjustment = ifelse((Surface_Headwind + Gust)/2 < 0, 0, ifelse((Surface_Headwind + Gust)/2 > 20, 20, (Surface_Headwind + Gust)/2)))
    dat_7 <- mutate(dat_7,
                    landing_adjustment = ifelse(Gust/2 < 10, 10, ifelse(Gust/2 > 20, 20, Gust/2)))
    dat_8 <- mutate(dat_8,
                    landing_adjustment = ifelse(Gust/2 < 0, 0, ifelse(Gust/2 > 20, 20, Gust/2)))
    dat_9 <- mutate(dat_9,
                    landing_adjustment = ifelse((Surface_Headwind + Gust) > 20, 15, ifelse((Surface_Headwind + Gust) > 10, 10, 5)))

    dat_com <- rbind(dat_0_10_11_12, dat_1_2_3_4, dat_5, dat_6, dat_7, dat_8, dat_9) %>%
               arrange(Follower_Flight_Plan_ID) %>%
               mutate(landing_adjustment_boeing = ifelse(Surface_Headwind/2 < 5, 5, ifelse(Surface_Headwind/2 > 20, 20, Surface_Headwind/2)))
