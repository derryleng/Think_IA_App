...
readme_txt <- "eTBS Performance Analysis Script Run
Script version: v5.2
Sprint task: IA-155
User: Derry
...
# Use database or local copy of vw_eTBS_Performance_Model and vw_Flying_Time_Analysis
use_database <- F
...
local_data_folder <- "LVNL Exports 2020-09-07"
...
# Enable use of legacy wake category columns
use_legacy_wake <- T
uk6cat_to_icao <- T
...
if (!use_database) {
  ...
  rd_dir <- file.path("Reference", local_data_folder, "rd.csv")
...
dbs_wake_turbulence <- as.data.table(sqlQuery(con, "SELECT * FROM tbl_DBS_Wake_Turbulence"))

aircraft_type_to_wake_legacy <- as.data.table(sqlQuery(con, "SELECT * FROM tbl_Aircraft_Type_To_Wake_Legacy"))

pm_query_str <- "SELECT * FROM vw_eTBS_Performance_Model"

if (use_database) {
  if (!is.na(date_filter)) {
    pm <- as.data.table(sqlQuery(con, sprintf("%s WHERE FP_Date IN ('%s')", pm_query_str, paste(date_filter, collapse = "','"))))
  } else {
    pm <- as.data.table(sqlQuery(con, pm_query_str))
  }
} else {
  pm <- fread(pm_dir)
}
...
rd_query_str <- "
    SELECT
        ...
        Surface_Wind_SPD,
        Surface_Wind_HDG,
        Flight_Plan_ID AS Follower_Flight_Plan_ID,
...
if (use_database) {
  if (!is.na(date_filter)) {
    rd <- as.data.table(sqlQuery(con, sprintf("%s WHERE FTA.FP_Date IN ('%s')", rd_query_str, paste(date_filter, collapse = "','"))))
  } else {
    rd <- as.data.table(sqlQuery(con, rd_query_str))
  }
} else {
  rd <- fread(rd_dir)
}
...
if (!is.na(date_filter)) {
  pre_pm_nrow <- nrow(pm)
  pm <- pm[FP_Date %in% date_filter]
  post_pm_nrow <- nrow(pm)
  message("Removed ", pre_pm_nrow - post_pm_nrow, " out of ", pre_pm_nrow, " performance model rows outside of specified date(s).")
}
...
#post_pm_nrow <- nrow(pm)
#message("Removed ", pre_pm_nrow - post_pm_nrow, " out of ", pre_pm_nrow, " performance model rows with NA Recat_eTBS_0DME_Wake_Separation_Distance.")
...
# Update legacy wake categories
pm$Leader_UK_Wake_Cat <- aircraft_type_to_wake_legacy[match(pm$Leader_Aircraft_Type, Aircraft_Type)]$Wake
pm$Follower_UK_Wake_Cat <- aircraft_type_to_wake_legacy[match(pm$Follower_Aircraft_Type, Aircraft_Type)]$Wake

# Update legacy wake separation distances (with 3NM MRS)
pm$UK6Cat_Separation_Distance <- dbs_wake_turbulence[match(paste(pm$Leader_UK_Wake_Cat, pm$Follower_UK_Wake_Cat), paste(Leader_WVI, Follower_WVI))]$WT_Separation_Distance / 1852
pm$UK6Cat_Separation_Distance <- ifelse(is.na(pm$UK6Cat_Separation_Distance), radar_min_sep, pm$UK6Cat_Separation_Distance)

# 0DME Separation Accuracy
# pm$Observed_Legacy_0DME_Separation_Accuracy <- pm$Observed_0DME_Separation_Distance - pm$UK6Cat_Separation_Distance

# Save merged pm
fwrite(pm, file.path(outdir, "pm_merged.csv"))
...
if (use_legacy_wake) {
  ...
  # Calculate current applied accuracy using UK 6 CAT
  ...
  # Calculate current applied accuracy using Recat
  pm$Observed_4DME_Separation_Accuracy_New <- pm$Observed_4DME_Separation_Distance - pm$Recat_eTBS_4DME_Wake_Separation_Distance

}

# ---
# NEED TO ADD BOUNDED ALL SEPARATION VERSIONS
# ---
# RECAT Separation at 1DME set 2.5NM lower bound
# Should we be removing this bound if considering MRS with All pairs?
...
# RECAT All Separation at 1DME set 2.5NM lower bound
pm$Recat_eTBS_0DME_All_Separation_Distance_New <- ifelse(pm$Recat_eTBS_0DME_All_Separation_Distance < 2.5, 2.5, pm$Recat_eTBS_0DME_All_Separation_Distance)
pm$Recat_eTBS_0DME_All_Separation_Distance_New_Rounded <- round(pm$Recat_eTBS_0DME_Wake_Separation_Distance_New, 1)
...
# Calculate RECAT All separation at 4DME and set 3NM lower bound
pm$Recat_eTBS_4DME_All_Separation_Distance_New <- pm$Recat_eTBS_0DME_All_Separation_Distance_New + pm$Forecast_ORD_TBS_Compression_New
pm$Recat_eTBS_4DME_All_Separation_Distance_New <- ifelse(pm$Recat_eTBS_4DME_All_Separation_Distance_New < 3, 3, pm$Recat_eTBS_4DME_All_Separation_Distance_New)
...
## NEED TO UPDATE SEPARATION ACCURACY ADJUSTMENT HERE
...
} else { # changed this to incorporate ICAO DBS accuracy
  
  pm2_non <- pm2[(Observed_4DME_Separation_Distance - UK6Cat_DBS_4DME_Wake_Separation_Distance) < 2 & Leader_Aircraft_Type != "A388"]
  pm2_A388 <- pm2[(Observed_4DME_Separation_Distance - UK6Cat_DBS_4DME_Wake_Separation_Distance) < 3 & Leader_Aircraft_Type == "A388"]
  pm3 <- rbind(pm2_non, pm2_A388)
...
# Use UK6Cat TBS Distance/Ref time if using Legacy (UK) and Use UK6Cat DBS Distance (Set to ICAO) and DBS_1DME_Time_Separation (Perfect ICAO) if not
if (legacy_wake == T){
...
data1 <- mutate(data1,
                UK_actual_1DME_Time_Accuracy = Observed_1DME_Separation_Time - UK6Cat_DBS_1DME_Time_Separation,
                UK_actual_1DME_Distance_Accuracy = Observed_1DME_Separation_Distance - UK6Cat_DBS_4DME_Wake_Separation_Distance,
                UK_actual_4DME_Distance_Accuracy = Observed_4DME_Separation_Distance - UK6Cat_DBS_4DME_Wake_Separation_Distance )
...
data1$Recat_eTBS_0DME_All_Separation_Distance_Cap <- ifelse(data1$Recat_eTBS_0DME_All_Separation_Distance_old < radar_min_sep, radar_min_sep, data1$Recat_eTBS_0DME_All_Separation_Distance_old)
data1$Forecast_ORD_TBS_Compression_Cap <- ifelse(data1$Forecast_ORD_TBS_Compression_old < 0, 0, data1$Forecast_ORD_TBS_Compression_old)
