...
Notes: Re-run of model after ROT updates made by IA-186
"
...
# Average IAS Filtering parameters
use_ias_filter <- T
...
All_Actual <- 4
A388_Actual <- 5
A388_Adj <- F # !!TURN THIS ON TO USE A388 ACTUAL!!
...
# MC Edit 5/10.  I think these need to be there regardless?  Otherwise
# data does not save
...
Output_To_Word <- function(doc, data, data2, scheme, time_required, sep_scheme, sep_scheme_2, plot_title_1, plot_title_2, x_title_1, x_title_2, plot_header, header){
  ...
  for (i in 1:nrow(sep_scheme)){
    ...
    if(time_required){time1 <- sep_scheme$RECAT_Time_Reference[i]
                      time2 <- sep_scheme_2$RECAT_Time_Reference[i]}
    ...
    if(nrow2 > 0){
      ...
      if( time_required ){ plot2 <- plot_time_histogram(plot_data_2, plot_title_2_new, x_title_2, time2, "light green") }
...
if (use_ias_filter){
  av_segs <- filter(test_segs, DME_Seg > (max_ias_filter_dme - 1)) %>% group_by(Landing_Pair_ID) %>% 
    summarise(Leader_Ave_Flight_IAS = mean(Leader_Average_IAS, na.rm=T),
              Follower_Ave_Flight_IAS = mean(Follower_Average_IAS, na.rm=T))
