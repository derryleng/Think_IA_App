...
# NOTE: set to NA for full dataset or specific date(s)
test_date <- "02/06/2015"
...
pm <- fread("Input/eTBS_Performance_Model.csv", na.strings = "NULL")

rd <- fread("Input/eTBS_Reference_Data.csv", na.strings = "NULL")
...
for (i in 1:nrow(rd_ave_spd)) {
  ...
  perc <- round(i/nrow(rd_ave_spd)*100, 0)
  if (perc %% 20 == 0) message("Merging average speeds - ", perc, "% complete.")
...
# ## Insert Runway Heading
# RWY_HDG <- ifelse(
#   abs(data1$Observed_AGI_Surface_Headwind-cos(NISTdegTOradian(data1$Observed_AGI_Surface_Wind_HDG-269.715))*data1$Observed_AGI_Surface_Wind_SPD)<0.01,
#   270,
#   ifelse(
#     abs(data1$Observed_AGI_Surface_Headwind-cos(NISTdegTOradian(data1$Observed_AGI_Surface_Wind_HDG-89.675))*data1$Observed_AGI_Surface_Wind_SPD)<0.01,
#     90,
#     NA)
# )
# data1 <- as.data.frame(append(data1, list(RWY_HDG = RWY_HDG), after = 22))
# rm(RWY_HDG)
# 
# ## Insert Crosswind
# Crosswind <- ifelse(data1$RWY_HDG == 90, sin(NISTdegTOradian(data1$Observed_AGI_Surface_Wind_HDG-89.675))*data1$Observed_AGI_Surface_Wind_SPD, ifelse(data1$RWY_HDG == 270, sin(NISTdegTOradian(data1$Observed_AGI_Surface_Wind_HDG-269.715))*data1$Observed_AGI_Surface_Wind_SPD,NA))
# data1 <- as.data.frame(append(data1, list(Observed_AGI_Surface_Crosswind = Crosswind), after = 24))
# rm(Crosswind)
...
sd_j_not_j <-  sd(j_not_j)

h_x <- pm[Leader_UK_Wake_Cat == "HEAVY" &
...
plot_time_histogram <- function(data, plot_title, line_value, fill_colour){
  ...
  if (data$Leader_Recat_Wake_Cat[1] == "SUPER"){ x_lim = c(40, 220)} else {x_lim = c(40, 180)}
...
fwrite(t1, file.path(outdir, "eTBS_1DME_Time_Accuracy_Wake_Type.csv"))

fwrite(t2, file.path(outdir, "UK6_1DME_Time_Accuracy_Wake_Type.csv"))
...
fwrite(t3, file.path(outdir, "eTBS_1DME_Time_Accuracy_Wake_Wake.csv"))

fwrite(t4, file.path(outdir, "UK6_1DME_Time_Accuracy_Wake_Wake.csv"))
...
fwrite(t5, file.path(outdir, "eTBS_1DME_Time_Accuracy_Stats_Wake_Type.csv"))

fwrite(t6, file.path(outdir, "UK6_1DME_Time_Accuracy_Stats_Wake_Type.csv"))
...
fwrite(t7, file.path(outdir, "eTBS_1DME_Time_Accuracy_Stats_Wake_Wake.csv"))

fwrite(t8, file.path(outdir, "UK6_1DME_Time_Accuracy_Stats_Wake_Wake.csv"))
