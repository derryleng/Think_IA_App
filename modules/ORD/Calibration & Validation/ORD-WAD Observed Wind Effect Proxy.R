...
#con <- Get_RODBC_Database_Connection(IP = "192.", Database = "NavCan_TBS_V3")
...
ORD_VV <- sqlQuery(con, "SELECT * FROM vw_ORD_Validation_View", stringsAsFactors = F)
WAD_VV <- sqlQuery(con, "SELECT * FROM vw_WAD_Validation_View", stringsAsFactors = F)
