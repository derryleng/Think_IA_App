...
unique_aircraft_types <- modeldata %>% dplyr::select(Follower_Aircraft_Type) %>% distinct()
...
missing_lss_type <- filter(unique_aircraft_types, is.na(landing_stabilisation_speed_type))
...
modeldata <- left_join(modeldata, dplyr::select(landing_adjustments, c(Follower_Flight_Plan_ID, landing_stabilisation_speed_type, landing_adjustment, landing_adjustment_boeing)), by = "Follower_Flight_Plan_ID")

modeldata <- modeldata %>% dplyr::rename(lss_type = landing_stabilisation_speed_type)
...
if (Operation == 'PWS') {
  ...
  aircraft_adaptation_int <- modeldata_filtered_a1 %>% group_by(Follower_Aircraft_Type) %>%
    ...
    dplyr::rename("Landing_Stabilisation_Speed_Type_Lead" = "landing_stabilisation_speed_type") %>%
    mutate(Landing_Stabilisation_Speed_Type_Follower = Landing_Stabilisation_Speed_Type_Lead,
           ...
           Min_Safe_Landing_Speed_Lead = vref_med,
           Min_Safe_Landing_Speed_Follower = vref_med,
           Min_Safe_Landing_Speed_Buffer_Follower = 0,
           ...
           Final_Deceleration_Lead = ifelse(b - a2 > 0 & n2 != n1, (b-a2)/(n2-n1), wake_decel),
           Final_Deceleration_Follower = ifelse(b - a2 > 0 & n2 != n1, (b-a2)/(n2-n1), wake_decel),
  ...
  type_out <- aircraft_adaptation_int %>% filter(N >= empirical_threshold |
    ...
    dplyr::select(-c(N, a1, b, a2, n1, n2, wake_decel, d2, vref_med, vref_percentile)) %>%
    dplyr::rename(Aircraft_Type = Follower_Aircraft_Type) %>%
    dplyr::select(Aircraft_Type, Compression_Commencement_Threshold, everything())

  wake_out <- wake_adaptation_int %>% dplyr::select(-c(N, a1, b, a2, n1, n2, wake_decel, d2, vref_med, vref_percentile))
