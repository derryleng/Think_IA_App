...
#                |                                                        #
# Purpose        |  Compares Vref values for currently being adapted      #
#                |  airport with previous calibration data                #
#                |                                                        #
# ----------------------------------------------------------------------- #
...
Ref_Adaptation <- fread(file.path(ref_data, paste0(Comparison_Airport_Code, "_ORD_Aircraft_Adaptation.csv")))

# New_Adaptation <- fread(file.path(Project_Directory, adaptation_folder, adaptation_file))
New_Adaptation <- fread(file.path(ord_dir, adaptation_folder, adaptation_file))
# New_Adaptation <- fread(file.path("C:\\Users\\Andy Hyde\\Dropbox (Think Research)\\NATS Projects\\NATS NavCanada TBS\\Data Analysis\\Outputs\\ORD Output\\Adaptation 24_02_21 Andy 5 THRESH\\Populate_tbl_ORD_Aircraft_Adaptation_CYYZ.csv"))

New_Adaptation_Wake <- fread(list.files(path = file.path(ord_dir, adaptation_folder), pattern = "Populate_tbl_ORD_Wake_Adaptation_[A-Z]{1,4}.csv", full.names = T)[1])
...
names(Aircraft_Type_Comparison) <- c("Aircraft_Type", Comparison_Airport_Code, Airport_Code)
Aircraft_Type_Comparison$Diff <- Aircraft_Type_Comparison[[Airport_Code]] - Aircraft_Type_Comparison[[Comparison_Airport_Code]]
...
png(file.path(out_dir, paste0("Vref Comparison Diff ", Airport_Code, " vs ", Comparison_Airport_Code, ".png")), width = 1750*5-550*5, height = 550*5, res = 360)
g1
dev.off()

# Plotted Comparison Aircraft Code Vrefs vs Airport_Code Vrefs
g2_vref_range <- range(c(Aircraft_Type_Comparison[[Comparison_Airport_Code]], Aircraft_Type_Comparison[[Airport_Code]]))
g2_lim_min <- round_any(g2_vref_range[1], 10, floor)
g2_lim_max <- round_any(g2_vref_range[2], 10, ceiling)

g2 <- ggplot(data = Aircraft_Type_Comparison, aes_string(x = Airport_Code, y = Comparison_Airport_Code)) +
  geom_point() +
...
png(file.path(out_dir, paste0("Vref Comparison Full ", Airport_Code, " vs ", Comparison_Airport_Code, ".png")), width = 1000, height = 800)
