...
Project_Directory <- file.path(Root_Dir[1], User_Dir[2], Proj_Dir[1]) # Select the correct index numbers!
...
con <- RODBC::odbcDriverConnect(connection=sprintf(
  ...
  "SQL Server", "192.168.1.32", "LVNL_UTMA_Validation", "ruser", "Th!nkruser"
))
...
# Name of output folder in project directory ---------------------------- #
outdir_parameter_summary <- "Adaptation - Sprint 13 - IA-187 v2"
...
# Name of output folder in project directory ---------------------------- #
outdir_validation_analysis <- "Validation - MC Test Sprint 12 New v3"
...
sep_adjust <- fread(file.path(Project_Directory, "separation_adjustment_parameters_v2.csv"))
