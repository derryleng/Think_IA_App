...
modeldata <- select(modeldata, c(-landing_adjustment, -landing_adjustment_boeing))

modeldata <- left_join(modeldata, select(landing_adjustments, c(Follower_Flight_Plan_ID, landing_adjustment, landing_adjustment_boeing)), by = "Follower_Flight_Plan_ID")


# for (i in 1:nrow(landing_adjustments)) {
# 
#   land_adj_i <- calc_landing_adjustment(landing_adjustments$landing_stabilisation_speed_type[i], landing_adjustments$Surface_Headwind[i])
#   land_adj_boeing_i <- calc_landing_adjustment(0, landing_adjustments$Surface_Headwind[i])
# 
#   if (!is.null(land_adj_i)) {
#     landing_adjustments$landing_adjustment[i] <-  land_adj_i
#   } 
#   else {
#     landing_adjustments$landing_adjustment[i] <-  0
#     message("No LSS Type found for ", landing_adjustments$Follower_Aircraft_Type)
#     
#   }
# 
#   if (!is.null(land_adj_boeing_i)) {
#     landing_adjustments$landing_adjustment_boeing[i] <-  land_adj_boeing_i
#   }
# }
...
#-------------------------------------------------------------------------#
#Filtering out anomalous values corresponding to go-arounds ---------------
#-------------------------------------------------------------------------#
...
# 
# modeldata <- filter(modeldata, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_a1 <- filter(modeldata_filtered_a1, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_a2 <- filter(modeldata_filtered_a2, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_b <- filter(modeldata_filtered_b, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_d <- filter(modeldata_filtered_d, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_n1 <- filter(modeldata_filtered_n1, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
# modeldata_filtered_n2 <- filter(modeldata_filtered_n2, !(Follower_Flight_Plan_ID %in% potential_anomalies$Flight_Plan_ID))
...
for (i in 1:length(actypes)) {
  ...
  #Asigning the predetermined values (from wrapper function)
  type_adaptation[[i]] <- type_adaptation_input_table
  
  #ASinging the type specific values
  ...
  # type_adaptation[[i]]$Initial_Deceleration_Lead <- d2
  # type_adaptation[[i]]$Initial_Deceleration_Follower <- d2
...
type_adaptation_out <- rbindlist(type_adaptation) %>% filter(Aircraft_Type %in% (unique_aircraft_types %>% filter(N >= empirical_threshold) %>% .$Follower_Aircraft_Type))
...
for (i in 1:length(wake_cats)) {
  ...
  wake_adaptation[[i]]$Min_Safe_Landing_Speed_Lead <- round(vref, 1)
  wake_adaptation[[i]]$Min_Safe_Landing_Speed_Follower <- max(round(vref + 5, 1), round(median(dat2$vref), 1), na.rm = T)
  wake_adaptation[[i]]$Final_Deceleration_Lead <- round(d, 1)
  wake_adaptation[[i]]$Final_Deceleration_Follower <- round(d, 1)
...
if (use_weighted_average) {
  ...
  # for (i in seq(3, 8, 1)) {
  #   
  #   wake_pairs_leader <- wake_pairs[DBS_Distance == i, .(Count = sum(Count)), by = Leader_WTC]
  #   Min_Safe_Landing_Speed_Lead_i <- sum(wake_adaptation[match(wake_pairs_leader$Leader_WTC, Wake_Cat)]$Min_Safe_Landing_Speed_Lead * wake_pairs_leader$Count) / sum(wake_pairs_leader$Count)
  #   Final_Deceleration_Lead_i <- sum(wake_adaptation[match(wake_pairs_leader$Leader_WTC, Wake_Cat)]$Final_Deceleration_Lead * wake_pairs_leader$Count) / sum(wake_pairs_leader$Count)
  #   Initial_Deceleration_Lead_i <- sum(wake_adaptation[match(wake_pairs_leader$Leader_WTC, Wake_Cat)]$Initial_Deceleration_Lead * wake_pairs_leader$Count) / sum(wake_pairs_leader$Count)
  #   
  #   wake_pairs_follower <- wake_pairs[DBS_Distance == i, .(Count = sum(Count)), by = Follower_WTC]
  #   Min_Safe_Landing_Speed_Follower_i <- sum(wake_adaptation[match(wake_pairs_follower$Follower_WTC, Wake_Cat)]$Min_Safe_Landing_Speed_Follower * wake_pairs_follower$Count) / sum(wake_pairs_follower$Count)
  #   Final_Deceleration_Foll_i <- sum(wake_adaptation[match(wake_pairs_follower$Follower_WTC, Wake_Cat)]$Final_Deceleration_Follower * wake_pairs_follower$Count) / sum(wake_pairs_follower$Count)
  #   Initial_Deceleration_Foll_i <- sum(wake_adaptation[match(wake_pairs_follower$Follower_WTC, Wake_Cat)]$Initial_Deceleration_Follower * wake_pairs_follower$Count) / sum(wake_pairs_follower$Count)
  #   
  ...
  #   
  #   #Setting the parameters not predefined
  #   dbs_adaptation_list[[i]]$DBS_Distance <- i
  #   dbs_adaptation_list[[i]]$Min_Safe_Landing_Speed_Lead <- Min_Safe_Landing_Speed_Lead_i
  #   dbs_adaptation_list[[i]]$Min_Safe_Landing_Speed_Follower <- Min_Safe_Landing_Speed_Follower_i
  #   dbs_adaptation_list[[i]]$Final_Deceleration_Lead <- Final_Deceleration_Lead_i
  #   dbs_adaptation_list[[i]]$Final_Deceleration_Follower <- Final_Deceleration_Foll_i
  ...
  # dbs_adaptation_list <- lapply(seq(3, 8, 1), function(i) {
  #   
  #   wake_pairs_leader <- wake_pairs[DBS_Distance == i, .(Count = sum(Count)), by = Leader_WTC]
  #   Min_Safe_Landing_Speed_Lead_i <- sum(wake_adaptation[match(wake_pairs_leader$Leader_WTC, Wake_Cat)]$Min_Safe_Landing_Speed_Lead * wake_pairs_leader$Count) / sum(wake_pairs_leader$Count)
  #   Final_Deceleration_Lead_i <- sum(wake_adaptation[match(wake_pairs_leader$Leader_WTC, Wake_Cat)]$Final_Deceleration_Lead * wake_pairs_leader$Count) / sum(wake_pairs_leader$Count)
  ...
  #   Initial_Deceleration_Foll_i <- sum(wake_adaptation[match(wake_pairs_follower$Follower_WTC, Wake_Cat)]$Initial_Deceleration_Follower * wake_pairs_follower$Count) / sum(wake_pairs_follower$Count)
  #   
  #   return(dbs_adaptation_list_int)
  #   
  #   return(
  #     data.table(
  #       DBS_Distance = i,
  ...
  #       Landing_Stabilisation_Speed_Type_Lead = 0,
  #       Landing_Stabilisation_Speed_Type_Follower = 0,
  #       Min_Safe_Landing_Speed_Lead = Min_Safe_Landing_Speed_Lead_i,
  #       Min_Safe_Landing_Speed_Follower = Min_Safe_Landing_Speed_Follower_i,
  #       Apply_Gusting_Lead = 1,
  #       Apply_Gusting_Follower = 1,
  #       Local_Stabilisation_Distance_Lead = 4,
  #       Local_Stabilisation_Distance_Follower = 4,
  #       Steady_Procedural_Speed_Lead = 160,
  #       Steady_Procedural_Speed_Follower = 162,
  #       Final_Deceleration_Lead = Final_Deceleration_Lead_i,
  #       Final_Deceleration_Follower = Final_Deceleration_Foll_i,
  #       End_Initial_Deceleration_Distance_Lead = 12,
  #       End_Initial_Deceleration_Distance_Follower = 12,
  #       Initial_Procedural_Speed_Lead = 160,
  #       Initial_Procedural_Speed_Follower = 162,
  #       Initial_Deceleration_Lead = Initial_Deceleration_Lead_i,
  #       Initial_Deceleration_Follower = Initial_Deceleration_Foll_i
  #     )
  #   )
  # })
