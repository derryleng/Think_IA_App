...
IAS_Query <- "SELECT rtpd.Flight_Plan_ID, rtpd.Track_Time, Mode_S_IAS 
...
#data1 <- filter(data1, Follower_Flight_Plan_ID %in% ias_faf$Flight_Plan_ID)
...
# ---------------------------------------------------------------------------------------------------------------------------------------- #
## -- Process to attempt to replace invalid follower Observed Wind Effect values
# ---------------------------------------------------------------------------------------------------------------------------------------- #
...
if (Use_Leader_WEs == T){
  ...
  data1 <- data1o
  # Moved Forecast ORD Compression calculation above to be used for calculating Observed Follower Wind Effect.
  ...
  # Load in Radar data for the Leader(s) aircraft where their follower has no Stop/Start RTT.
  leader_radar_nofol_query <- "SELECT * FROM vw_Radar_Track_Point_Derived WHERE 
Flight_Plan_ID IN (SELECT Leader_Flight_Plan_ID FROM vw_ORD_Validation_View WHERE Follower_Stop_RTT IS NULL OR Follower_Start_RTT IS NULL)"
  
  leader_radar_nofol <- sqlQuery(con, leader_radar_nofol_query, stringsAsFactors = F)
  
  # Get the Forecast Compression Start/End Distances for the follower aircraft corresponding to this leader. (Assume 4.5NM LST)
  data1 <- mutate(data1, Follower_Forecast_Start_Distance = ORD_Compression + 4.5 + ORD_Separation_Distance)
  
  # Separate observations with/without valid follower distances to threshold & remove the invalid observed follower wind effect
  data1_fol <- filter(data1, !is.na(Follower_Start_RTT) & !is.na(Follower_Stop_RTT))
  data1_nofol <- filter(data1, is.na(Follower_Start_RTT) | is.na(Follower_Stop_RTT)) %>% select(-Observed_Mean_Follower_Wind_Effect)
  
  # Do the Observed Calcs
  data1_nofol <- Get_Average_Observed_Mode_S_Parameters(data1_nofol, leader_radar_nofol, "Rename_Me", "Leader",
                                                        "Range", Start_Var = "Follower_Forecast_Start_Distance", End_Var = "ORD_Separation_Distance") %>%
    select(-Observed_Leader_Rename_Me_IAS) %>% rename(Observed_Mean_Follower_Wind_Effect = Observed_Leader_Rename_Me_Wind_Effect)
  
  # Check the sample sizes
  message("Sample sizes of valid Observed Mean Follower Wind Effects USING LEADER CENTRELINE:")
  print(nrow(filter(data1_nofol, !is.na(Observed_Mean_Follower_Wind_Effect))))
  message("Sample sizes of valid Observed Mean Follower Wind Effects :")
  print(nrow(filter(data1_fol, !is.na(Observed_Mean_Follower_Wind_Effect))))
  
  # Bind the datasets back together!
  data1 <- rbind(data1_fol, data1_nofol)
  
  message("Total Data Size (Including NA):")
  print(nrow(data1))
...
if (ORD_Compression_Type == "New"){
  ...
  # data1 <- mutate(data1, ORD_Mean_Follower_IAS_Adjusted = ORD_Mean_Follower_IAS + ord_follower_ias_adjustment)
