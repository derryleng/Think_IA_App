...
Get_Forecast_Compression <- function(gstl, gstf){
  ...
  # TEMP: Make Leader Flying Distance NA if >0 NA GSPD
  Leader_Stats 
  ...
  # Order gstf in descending section number
  #gstf <- mutate(gstf, Opposite_Section = 10000 - Section_Number)
  #gstf <- gstf[order(gstf$Landing_Pair_ID, gstf$Opposite_Section),]
  #gstf <- select(gstf, -c("Opposite_Section"))
  ...
  All_Stats <- full_join(Leader_Stats, Follower_Stats, by = c("Landing_Pair_ID"))
...
#ORD_GS_Profile_Leader <- filter(testgs, This_Pair_Role == "L")
#ORD_GS_Profile_Follower <- filter(testgs, This_Pair_Role == "F")
...
if (!Debug_Mode){
  Separation_Distances <- select(Landing_Pair_Reference, Landing_Pair_ID, ORD_Separation_Distance)
}

# LSTs for ORD Start and WAD End
Local_Stabilisation_Distances <- filter(ORD_Aircraft_Profile, This_Pair_Role == "L") %>%
  select(Landing_Pair_ID, Local_Stabilisation_Distance) %>% mutate(Local_Stabilisation_Distance = 4 * 1852) # TEMP
...
if (Test_Mode){
  ...
  ORD_Prediction_SQL_Query <- "SELECT 
                                ...
                                OP.Forecast_AGI_Surface_Headwind AS SQL_Forecast_Surface_Headwind,
  ...
  ZCOMP_ORD_Prediction <- mutate(ZCOMP_ORD_Prediction,
                                 ...
                                 DIFF_DBS_All_Sep_Distance = abs(DBS_All_Sep_Distance - SQL_DBS_All_Sep_Distance),
                                 DIFF_Prediction_Time = abs(Prediction_Time - SQL_Prediction_Time),
                                 DIFF_Forecast_Surface_Headwind = abs(Forecast_AGI_Surface_Headwind - SQL_Forecast_Surface_Headwind),
                                 DIFF_ORD_Separation_Distance = abs(ORD_Separation_Distance - SQL_ORD_Separation_Distance),
                                 FLAG_ORD_Mean_Leader_IAS = ifelse(DIFF_ORD_Mean_Leader_IAS > 0.0001, 1, 0),
                                 FLAG_ORD_Mean_Leader_IAS = ifelse(is.na(ORD_Mean_Leader_IAS) & !is.na(SQL_ORD_Mean_Leader_IAS), 1, FLAG_ORD_Mean_Leader_IAS),
                                 FLAG_ORD_Mean_Leader_IAS = ifelse(!is.na(ORD_Mean_Leader_IAS) & is.na(SQL_ORD_Mean_Leader_IAS), 1, FLAG_ORD_Mean_Leader_IAS),
                                 FLAG_ORD_Mean_Follower_IAS = ifelse(DIFF_ORD_Mean_Follower_IAS > 0.0001, 1, 0),
                                 FLAG_ORD_Mean_Follower_IAS = ifelse(is.na(ORD_Mean_Follower_IAS) & !is.na(SQL_ORD_Mean_Follower_IAS), 1, FLAG_ORD_Mean_Follower_IAS),
                                 FLAG_ORD_Mean_Follower_IAS = ifelse(!is.na(ORD_Mean_Follower_IAS) & is.na(SQL_ORD_Mean_Follower_IAS), 1, FLAG_ORD_Mean_Follower_IAS),
                                 FLAG_ORD_Mean_Leader_WE = ifelse(DIFF_ORD_Mean_Leader_WE > 0.0001, 1, 0),
                                 FLAG_ORD_Mean_Leader_WE = ifelse(is.na(Forecast_ORD_Mean_Leader_Wind_Effect) & !is.na(SQL_ORD_Forecast_Mean_Leader_Wind_Effect), 1, FLAG_ORD_Mean_Leader_WE),
                                 FLAG_ORD_Mean_Leader_WE = ifelse(!is.na(Forecast_ORD_Mean_Leader_Wind_Effect) & is.na(SQL_ORD_Forecast_Mean_Leader_Wind_Effect), 1, FLAG_ORD_Mean_Leader_WE),
                                 FLAG_ORD_Mean_Follower_WE = ifelse(DIFF_ORD_Mean_Follower_WE > 0.0001, 1, 0),
                                 FLAG_ORD_Mean_Follower_WE = ifelse(is.na(Forecast_ORD_Mean_Follower_Wind_Effect) & !is.na(SQL_ORD_Forecast_Mean_Follower_Wind_Effect), 1, FLAG_ORD_Mean_Follower_WE),
                                 FLAG_ORD_Mean_Follower_WE = ifelse(!is.na(Forecast_ORD_Mean_Follower_Wind_Effect) & is.na(SQL_ORD_Forecast_Mean_Follower_Wind_Effect), 1, FLAG_ORD_Mean_Follower_WE),
                                 FLAG_ORD_Compression = ifelse(DIFF_ORD_Compression > 0.01, 1, 0),
                                 FLAG_ORD_Compression = ifelse(is.na(ORD_Forecast_Compression) & !is.na(SQL_ORD_Compression), 1, FLAG_ORD_Compression),
                                 FLAG_ORD_Compression = ifelse(!is.na(ORD_Forecast_Compression) & is.na(SQL_ORD_Compression), 1, FLAG_ORD_Compression),
                                 FLAG_DBS_All_Sep_Distance = ifelse(DIFF_DBS_All_Sep_Distance > 0.001, 1, 0),
                                 FLAG_DBS_All_Sep_Distance = ifelse(is.na(DBS_All_Sep_Distance) & !is.na(SQL_DBS_All_Sep_Distance), 1, FLAG_DBS_All_Sep_Distance),
                                 FLAG_DBS_All_Sep_Distance = ifelse(!is.na(DBS_All_Sep_Distance) & is.na(SQL_DBS_All_Sep_Distance), 1, FLAG_DBS_All_Sep_Distance),
                                 FLAG_Prediction_Time = ifelse(DIFF_Prediction_Time > 0, 1, 0),
                                 FLAG_Prediction_Time = ifelse(is.na(Prediction_Time) & !is.na(SQL_Prediction_Time), 1, FLAG_Prediction_Time),
                                 FLAG_Prediction_Time = ifelse(!is.na(Prediction_Time) & is.na(SQL_Prediction_Time), 1, FLAG_Prediction_Time),
                                 FLAG_Forecast_Surface_Headwind = ifelse(DIFF_Forecast_Surface_Headwind > 0.001, 1, 0),
                                 FLAG_Forecast_Surface_Headwind = ifelse(is.na(Forecast_AGI_Surface_Headwind) & !is.na(SQL_Forecast_Surface_Headwind), 1, FLAG_Forecast_Surface_Headwind),
                                 FLAG_Forecast_Surface_Headwind = ifelse(!is.na(Forecast_AGI_Surface_Headwind) & is.na(SQL_Forecast_Surface_Headwind), 1, FLAG_Forecast_Surface_Headwind),
                                 FLAG_ORD_Separation_Distance = ifelse(DIFF_ORD_Separation_Distance > 0.01, 1, 0),
                                 FLAG_ORD_Separation_Distance = ifelse(is.na(ORD_Separation_Distance) & !is.na(SQL_ORD_Separation_Distance), 1, FLAG_ORD_Separation_Distance),
                                 FLAG_ORD_Separation_Distance = ifelse(!is.na(ORD_Separation_Distance) & is.na(SQL_ORD_Separation_Distance), 1, FLAG_ORD_Separation_Distance))
  ...
  ZSTAT_ORD_Prediction <- group_by(ZCOMP_ORD_Prediction, Landing_Pair_Date) %>% summarise(
                                    ...
                                    CNT_Forecast_Surface_Headwind = sum(FLAG_Forecast_Surface_Headwind, na.rm = T),
  ...
  test <- filter(ZCOMP_ORD_Prediction, FLAG_ORD_Compression == 1)
  testids <- test$Landing_Pair_ID
  testgs <- filter(ORD_GS_Profile, Landing_Pair_ID %in% testids)
