...
prop_str <-  paste("C:\\Users\\", user, "\\Dropbox (Think Research)\\NATS Projects\\NATS NavCanada TBS\\Data Analysis\\Outputs\\Sample_Weighting_Output\\v2.2 24.02.21\\Wake Pair Proportions.csv", sep = "")
prop <- fread(prop_str)

#Arrival totals - update folder name
arr_str <- paste("C:\\Users\\", user, "\\Dropbox (Think Research)\\NATS Projects\\NATS NavCanada TBS\\Data Analysis\\Outputs\\Sample_Weighting_Output\\v2.2 24.02.21\\Arrival Totals.csv", sep = "")
arr <- fread(arr_str)
...
# Raw Tables 3: Queued Arrivals per Hour (Replaced with different CI estimation)
...
PR_Arr <- Indicator_Join(PR_Sep) %>% Performance_Table_Indicator_CI()
...
plt_mean <- ggplot() +
        geom_point(data=ggdata, aes(x = `Surface Wind`, y = Rate,colour=Series, group = Series), size =2)+
        geom_line(data=ggdata, aes(x = `Surface Wind`, y = Rate,colour=Series, group = Series), size = 1)+
        ggtitle(">0.5NM rate with Confidence Intervals")+
        labs(x = "Surface Wind Band (kt)", y = ">0.5NM Error Rate")+
        #coord_cartesian(ylim=c(-0.3,0.3))+
        scale_y_continuous(labels = scales::scientific)+
        scale_colour_brewer(type = "seq", palette = "Paired", direction = -1)+
        theme(axis.line = element_line(colour = "black",size = 0.5, linetype = "solid"), plot.title =  element_text(size=16), legend.title=element_blank(),axis.text.x = element_text(size=14),axis.text.y = element_text(size=14))
