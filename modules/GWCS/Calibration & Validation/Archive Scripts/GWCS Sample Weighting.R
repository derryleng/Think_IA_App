...
base_dir <- paste0("C:/Users/", user, "/Dropbox (Think Research)/NATS Projects/NATS NavCanada TBS/Data Analysis")

out_data <- file.path(base_dir, "Outputs/Sample_Weighting_Output", out_dirname)
if (!dir.exists(out_data)) dir.create(out_data)

ref_data <- file.path(base_dir, "Inputs/Sample_Weighting_Reference_Data")

# out_data <- file.path(base_dir, "Outputs/Sample_Weighting_Output", out_dirname, "Analysis Results")
# if (!dir.exists(out_data)) dir.create(out_data)
setwd(out_data)

db_data <- dirname(out_data)

# Set IP 192.168.1.39 or 192.168.1.23
