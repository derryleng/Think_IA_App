...
if (get_connection == T){
  con <- RODBC::odbcDriverConnect(connection=paste0("Driver={SQL Server};Server={192.168.1.39};Database={",database,"};Uid={ruser};Pwd={Th!nkruser};"))
}

# Version (determines output directory - must be string).
version <- "4.0.6 (151120)"
...
adap_set <- 2 #1 for Surface wind group, 2 for Surface Headwind safety case
...
Get_Speeds <- function(df, start, min, max, halves){
  ...
  dfn <- select(df, Flight_Plan_ID, Wake_Cat, Follower_ICAO4_WTC, Aircraft_Type, Follower_Aircraft_Class, 
...
Create_RECAT_seps <- function(summary){
  ...
  seps <- right_join(summary, all_seps, by=c("Wake_Cat"="Follower_WTC",
                                                   "Separation_Distance"="Ref_Distance")) %>%
...
Create_Wake_Adaptation <- function(seps){
  ...
  wake <- wake[order(wake$Leader_WTC, wake$Follower_WTC),]
  return(wake)
}

Create_ROT_Adaptation <- function(seps){
  ...
  rot <- rot[order(rot$Runway, rot$Leader_WTC, rot$Follower_WTC),]
  return(rot)
}
...
TBS_Values <- c(3, 4, 5, 6, 7, 8) #For LVNL. NAV require 3.5
...
IAS_Summary_Recat_SW_Seps <- Create_RECAT_seps(IAS_Summary_Recat_SW) %>% filter(Surface_Wind_Gp == "(0,3]")
IAS_Summary_Recat_SHW_Seps <- Create_RECAT_seps(IAS_Summary_Recat_SHW) %>% filter(Surface_Headwind_Gp == "(-5,5]")
...
wake_base_1 <- Create_Wake_Adaptation(IAS_Summary_Recat_SW_Seps) %>% select(-c("Surface_Wind_Gp"))
wake_base_2 <- Create_Wake_Adaptation(IAS_Summary_Recat_SHW_Seps) %>% select(-c("Surface_Headwind_Gp"))
...
ROT_base_1 <- Create_ROT_Adaptation(IAS_Summary_Recat_SW_Seps) %>% select(-c("Surface_Wind_Gp"))
ROT_base_2 <- Create_ROT_Adaptation(IAS_Summary_Recat_SHW_Seps) %>% select(-c("Surface_Headwind_Gp"))
...
IAS_Summary_Recat_SW_0_Seps <- Create_RECAT_seps(IAS_Summary_Recat_SW_0) %>% filter(Surface_Wind_Gp == "(0,3]") %>% 
  filter(Constraint == "Wake", Runway == "R04") %>% select(Leader_WTC, Follower_WTC, Count, Flying_Time) %>% mutate(Flying_Time = round(Flying_Time, 0))
...
IAS_Summary_Recat_SHW_0_Seps <- Create_RECAT_seps(IAS_Summary_Recat_SHW_0) %>% filter(Surface_Headwind_Gp == "(-5,5]") %>% 
  filter(Constraint == "Wake", Runway == "R04") %>% select(Leader_WTC, Follower_WTC, Count, Flying_Time) %>% mutate(Flying_Time = round(Flying_Time, 0))
...
IAS_Summary_Recat_All_0_Seps <- Create_RECAT_seps(IAS_Summary_Recat_All_0) %>% 
  filter(Constraint == "Wake", Runway == "R04") %>% select(Leader_WTC, Follower_WTC, Count, Flying_Time) %>% mutate(Flying_Time = round(Flying_Time, 0))
...
IAS_Summary_Recat_SHW_1_Seps <- Create_RECAT_seps(IAS_Summary_Recat_SHW_1) %>% filter(Surface_Headwind_Gp == "(-5,5]") %>% 
  filter(Constraint == "Wake", Runway == "R04") %>% select(Leader_WTC, Follower_WTC, Count, Flying_Time) %>% mutate(Flying_Time = round(Flying_Time, 0))
