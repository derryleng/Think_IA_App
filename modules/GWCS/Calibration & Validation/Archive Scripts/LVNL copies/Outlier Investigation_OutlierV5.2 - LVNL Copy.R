...
out_data <-paste0("C:\\Users\\",user,"\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\Output\\Outlier Investigation\\")
if (!dir.exists(out_data)) dir.create(out_data)  

#working directory
wd<- paste("C:\\Users\\", user,"\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts", sep="")
setwd(wd)
...
for (sepdist in 3:8){
        gwcs_file <-  paste(wd,"\\LVNL Data\\vw_Mode_S_Wind_Forecast_", sepdist, "nm.csv", sep = "")
...
plotdata <- data.frame(gwcs_errors_unique$Flight_Plan_ID, gwcs_errors_unique$FP_Date,gwcs_errors_unique$Time_At_4DME,gwcs_errors_unique$ForecastGSPD_ObservedGSPD_Mode_S_Error,gwcs_errors_unique$Callsign,gwcs_errors_unique$Sep_Dist,gwcs_errors_unique$Landing_Runway.x, gwcs_errors_unique$Forecast_GWCS_Error_Distance, gwcs_errors_unique$Surface_Wind_SPD)
...
for (i in (1:20)) {
  ...
  rawsegs_selection <- subset(rawsegs, FP_Date.x==errordate & Time_At_4DME.x<(time1+900) & Time_At_4DME.x>(time1-900)&DME_Seg %%2==0 & DME_Seg<=6 & Landing_Runway.x == RW)
  ...
  rawsegs_selection_by_flight <- subset(rawsegs, FP_Date.x==errordate & Time_At_4DME.x<(time1+500) & Time_At_4DME.x>(time1-500)&DME_Seg %%2==0 & DME_Seg<=10 & Landing_Runway.x == RW)
  #subset tables for graphs, based on variables above
  gwcsperformanceselection <- subset(gwcsperformance, Sep_Dist==dmeseg & FP_Date==errordate & Time_At_4DME<(time1+900) & Time_At_4DME>(time1-900) & Landing_Runway.x == RW)
