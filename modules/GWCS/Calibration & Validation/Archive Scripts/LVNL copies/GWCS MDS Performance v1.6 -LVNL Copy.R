...
# Version No.    |  1.5                                                   #
#                |                                                        #
# Date Modified  |  09/07/2020                                            #
#                |                                                        #
# Author(s)      |  Michael Cowham, Catherine Mason                       #
#                |                                                        #
# Project        |  eTBS Related Projects (LVNL)                          #
#                |                                                        #
...
# Output directory
out_data <-paste0("C:\\Users\\",user,"\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\Output\\",version)
...
# Load Data --------------------------------------------------------------
# ----------------------------------------------------------------------- #
# Wake pair proportions
prop_str <-  paste("C:\\Users\\", user, "\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\LVNL Data\\Wake Pair Proportions.csv", sep = "")
prop <- fread(prop_str)

#Arrival totals
arr_str <- paste("C:\\Users\\", user, "\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\LVNL Data\\Arrival Totals.csv", sep = "")
arr <- fread(arr_str)
...
for (sepdist in 3:8){
        gwcs_file <-  paste("C:\\Users\\", user, "\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\LVNL Data\\vw_Mode_S_Wind_Forecast_", sepdist, "nm.csv", sep = "")
...
con <- odbcDriverConnect(connection=sprintf(
        ...
        "SQL Server", "192.168.1.39", database, "ruser", "Th!nkruser"
))
...
flights_greater_200 <- fread(paste0("C:\\Users\\",user,"\\Dropbox (Think Research)\\NATS Projects\\NATS LVNL Schiphol\\Phase 2\\20. LVNL Scripts\\GWCS_Scripts\\Output\\Outlier Investigation\\flights_greater_200.csv"))
...
gwcs_data_proc$qc_flag <- ifelse(gwcs_data_proc$Landing_Runway %in% c('R18R','R06','R24','R09','R27','R36R','R18C','R36C','R22') & 
...
# Create runway field
gwcs_data_proc$Landing_Runway_Dir <- factor(substr(as.character(gwcs_data_proc$Landing_Runway),1,3))
...
Performance_Table_Operational <- function(data){
        data <- mutate(data,
                          `O-F >10kts`=data$`O-F >10kts` *qwph,
                          `O-F >20kts`=data$`O-F >20kts` *qwph,
                          `O-F >0.25NM`=data$`O-F >0.25NM` *qwph,
                          `O-F >0.5NM`=data$`O-F >0.5NM` *qwph,
                          `O-F >1.0NM`=data$`O-F >1.0NM` *qwph)
        return(data)
}


# Convert to DF for ease of manipulation
perf_df <- data.frame(gwcs_data_anem) %>% rename(`Surface Wind`=surface_wind_band, `Runway Direction`=Landing_Runway_Dir)
...
rw_groups <- c("R18", "R36", "R22", "R27", "R06", "R24")
...
PR_Sep_LR <- perf_df %>% group_by(Sep_Dist, `Runway Direction`) %>% Performance_Table() %>% filter(`Runway Direction` %in% rw_groups)
...
PR_Ind <- Indicator_Join(PR_Sep) %>% Performance_Table_Indicator()
PR_Ind_SW <- Indicator_Join(PR_Sep_SW) %>% group_by(`Surface Wind`) %>% Performance_Table_Indicator()
PR_Ind_LR <- Indicator_Join(PR_Sep_LR) %>% group_by(`Runway Direction`) %>% Performance_Table_Indicator()
PR_Ind_SW_LR <- Indicator_Join(PR_Sep_SW_LR) %>% group_by(`Surface Wind`, `Runway Direction`) %>% Performance_Table_Indicator() %>% arrange(`Runway Direction`)

# Raw Tables 3: Queued Arrivals per Hour
PR_Arr <- Performance_Table_Operational(PR_Ind)
PR_Arr_SW <- Performance_Table_Operational(PR_Ind_SW)
PR_Arr_LR <- Performance_Table_Operational(PR_Ind_LR)
...
fwrite(PR_Sep, file = paste(out_data, "\\Performance Results, Sep Dist.csv", sep = ""))
...
for (group in SW_groups){
        PRS <- filter(PR_Sep_SW, `Surface Wind` == group) %>% select(-`Surface Wind`)
        fwrite(PRS, file = paste(out_data, paste0("\\Performance Results, Sep Dist, Surface Wind, ", group, ".csv"), sep =""))
}

for (group in rw_groups){
        PRS <- filter(PR_Sep_LR, `Runway Direction` == group) %>% select(-`Runway Direction`)
        fwrite(PRS, file = paste(out_data, paste0("\\Performance Results, Sep Dist, Runway Direction, ", group, ".csv"), sep = ""))
}

# Investigation Tables: Not Weighted
PR_SW <- perf_df %>% group_by(`Surface Wind`) %>% Performance_Table() %>% filter(`Surface Wind` %in% SW_groups)
PR_SW_LR <- perf_df %>% group_by(`Surface Wind`, `Runway Direction`) %>% Performance_Table() %>% filter(`Surface Wind` %in% SW_groups,
                                                                                                       `Runway Direction` %in% rw_groups)


PR_Sep_SW_LR <- perf_df %>% group_by(Sep_Dist, `Surface Wind`, `Runway Direction`) %>% Performance_Table() %>% filter(`Surface Wind` %in% SW_groups,
                                                                                                        `Runway Direction` %in% rw_groups)

PR_Sep_SW_LR_Test <- filter(PR_Sep_SW_LR, `O-F >0.5NM Rate`>0)

PR_SW_LR_Low <- filter(PR_SW_LR, `Surface Wind` == "[0,4)")
PR_SW_LR_4 <- filter(PR_SW_LR, `Surface Wind` == "[4,9)")




############################ For Anemometer Data - OLD###########################
# Create separate dataframe for mergerd wind data due to loss of rows in join
# Create the fields to lookup the Anemometer Data
#anem <- plyr::rename(anem, c("Anemo_Date" = "FP_Date", "Anemo_Time" = "Time_At_4DME_60s"))
# anem <- plyr::rename(anem, c("Anemo_Date" = "FP_Date"))
# anem$Time_At_4DME_60s <- round(60 * floor(anem$Anemo_Time / 60), digits = 0)
# gwcs_data$Time_At_4DME_60s <- round(60 * floor(gwcs_data$Time_At_4DME / 60), digits = 0)
# #gwcs_data <- merge(gwcs_data, anem, by = c("FP_Date", "Time_At_4DME_60s", "Landing_Runway"))
# gwcs_data_wind <- merge(gwcs_data, anem, by = c("FP_Date", "Time_At_4DME_60s", "Landing_Runway"))
# gwcs_data_wind$Anemo_SPD <- as.numeric(gwcs_data_wind$Anemo_SPD)
# gwcs_data_wind$Anemo_HDG <- as.numeric(gwcs_data_wind$Anemo_HDG)
# 
# #group by surface wind bands
# gwcs_data_wind$surface_wind_band <- cut(gwcs_data_wind$Anemo_SPD, c(0, 4, 9, 14, Inf), right = FALSE)
# 

# # ######################## For METAR Data ##################################
# met$Date <- as.Date(gsub("^([0-9-]+) .*$", "\\1", met$valid))
# met$Time <- paste0(met$hour, ":", met$min, ":00")
# met$Time_secs <- met$hour*3600 + met$min*60
# #Filter METAR for only columns want to join
# met_filt <- subset(met,select=c("Date","Time","Time_secs","wind_hdg","wind_speed"))
# met_filt <- plyr::rename(met_filt, c("Date" = "FP_Date", "Time_secs" = "Time_At_4DME_new"))
# #Translate time at 4DME to a time either 20/50 minutes past the hour
# gwcs_data$Time_At_4DME_new <- gwcs_data$Time_At_4DME + 600
# gwcs_data$Time_At_4DME_new <- 1800 * round(gwcs_data$Time_At_4DME_new / 1800, digits=0)
# gwcs_data$Time_At_4DME_new <- gwcs_data$Time_At_4DME_new - 600
# #Merge METAR data onto gwcsdata
# gwcs_data <- merge(gwcs_data,met_filt, by=c("FP_Date","Time_At_4DME_new"))
# #group pby surface wind band
# gwcs_data_met$surface_wind_band <- cut(gwcs_data_met$wind_speed, c(0, 4, 9, 14, Inf), right = FALSE)
...
# Create a table of these flags by Separation Distance
a1 <- ddply(gwcs_data_anem, "Sep_Dist", summarise,
            Ntot = n(),
            N = sum(stale_flag, na.rm = TRUE),
            MeanWE = mean(Forecast_Wind_Effect_IAS, na.rm = TRUE),
            Mean = mean(Forecast_Wind_Effect_IAS_Error, na.rm = TRUE),
            StdeV = sd(Forecast_Wind_Effect_IAS_Error, na.rm = TRUE),
            gt_10kt = sum(gt_10kt, na.rm = TRUE),
            gt_20kt = sum(gt_20kt, na.rm = TRUE),
            gt_25nm = sum(gt_25nm, na.rm = TRUE),
            gt_50nm = sum(gt_50nm, na.rm = TRUE),
            gt_100nm = sum(gt_100nm, na.rm = TRUE))

gwcs_perf <- as.data.frame(a1)

gwcs_perf$gt_10kt_rate <- gwcs_perf$gt_10kt / gwcs_perf$N
gwcs_perf$gt_20kt_rate <- gwcs_perf$gt_20kt / gwcs_perf$N
gwcs_perf$gt_25nm_rate <- gwcs_perf$gt_25nm / gwcs_perf$N
gwcs_perf$gt_50nm_rate <- gwcs_perf$gt_50nm / gwcs_perf$N
gwcs_perf$gt_100nm_rate <-gwcs_perf$gt_100nm / gwcs_perf$N

# Create a table of these flags by Separation Distance and Landing Runway
a2 <- ddply(gwcs_data_anem, c("Sep_Dist", "Landing_Runway_Dir"), summarise, 
            Ntot = n(),
            N = sum(stale_flag, na.rm = TRUE),
            MeanWE = mean(Forecast_Wind_Effect_IAS, na.rm = TRUE),
            Mean = mean(Forecast_Wind_Effect_IAS_Error, na.rm = TRUE),
...
gwcs_perf_rw <- as.data.frame(a2)

gwcs_perf_rw$gt_10kt_rate <- gwcs_perf_rw$gt_10kt / gwcs_perf_rw$N
gwcs_perf_rw$gt_20kt_rate <- gwcs_perf_rw$gt_20kt / gwcs_perf_rw$N
gwcs_perf_rw$gt_25nm_rate <- gwcs_perf_rw$gt_25nm / gwcs_perf_rw$N
gwcs_perf_rw$gt_50nm_rate <- gwcs_perf_rw$gt_50nm / gwcs_perf_rw$N
gwcs_perf_rw$gt_100nm_rate <- gwcs_perf_rw$gt_100nm / gwcs_perf_rw$N

# Create a table of these flags by GWCS Wind Conditions
a3 <- ddply(gwcs_data_anem, c("Sep_Dist", "gwcs_band"), summarise, 
            Ntot = n(),
            N = sum(stale_flag, na.rm = TRUE),
            MeanWE = mean(Forecast_Wind_Effect_IAS, na.rm = TRUE),
            Mean = mean(Forecast_Wind_Effect_IAS_Error, na.rm = TRUE),
...
gwcs_perf_gwcs <- as.data.frame(a3)

gwcs_perf_gwcs$gt_10kt_rate <- gwcs_perf_gwcs$gt_10kt / gwcs_perf_gwcs$N
gwcs_perf_gwcs$gt_20kt_rate <- gwcs_perf_gwcs$gt_20kt / gwcs_perf_gwcs$N
gwcs_perf_gwcs$gt_25nm_rate <- gwcs_perf_gwcs$gt_25nm / gwcs_perf_gwcs$N
gwcs_perf_gwcs$gt_50nm_rate <- gwcs_perf_gwcs$gt_50nm / gwcs_perf_gwcs$N
gwcs_perf_gwcs$gt_100nm_rate <- gwcs_perf_gwcs$gt_100nm / gwcs_perf_gwcs$N

gwcs_perf_gwcs <- gwcs_perf_gwcs[order(gwcs_perf_gwcs$gwcs_band),]

# Create a table of these flags by Surface Wind Conditions
a4 <- ddply(gwcs_data_anem, c("Sep_Dist", "surface_wind_band"), summarise,
            Ntot = n(),
            N = sum(stale_flag, na.rm = TRUE),
            MeanWE = mean(Forecast_Wind_Effect_IAS, na.rm = TRUE),
            Mean = mean(Forecast_Wind_Effect_IAS_Error, na.rm = TRUE),
...
gwcs_perf_surf <- as.data.frame(a4)

gwcs_perf_surf$gt_10kt_rate <- gwcs_perf_surf$gt_10kt / gwcs_perf_surf$N
gwcs_perf_surf$gt_20kt_rate <- gwcs_perf_surf$gt_20kt / gwcs_perf_surf$N
gwcs_perf_surf$gt_25nm_rate <- gwcs_perf_surf$gt_25nm / gwcs_perf_surf$N
gwcs_perf_surf$gt_50nm_rate <- gwcs_perf_surf$gt_50nm / gwcs_perf_surf$N
gwcs_perf_surf$gt_100nm_rate <- gwcs_perf_surf$gt_100nm / gwcs_perf_surf$N

gwcs_perf_surf <- gwcs_perf_surf[order(gwcs_perf_surf$surface_wind_band), ]

# Write the results to .csv files
fwrite(gwcs_perf, file = paste(out_data, "\\GWCS_Performance_Stats_All.csv", sep = ""))
       
fwrite(gwcs_perf_rw, file = paste(out_data, "\\GWCS_Performance_Stats_Runway.csv", sep = ""))

#write.csv(gwcs_perf_gwcs, file = paste(out_data, "\\GWCS_Performance_Stats_GWCS.csv", sep = ""))

#Remove NA surface wind band values
gwcs_perf_surf <- gwcs_perf_surf[!is.na(gwcs_perf_surf$surface_wind_band),]
fwrite(gwcs_perf_surf, file = paste(out_data, "\\GWCS_Performance_Stats_Surface_Wind.csv", sep = ""))
...
# Overall Performance
plt_mean <- ggplot() +
        geom_point(data=gwcs_perf, aes(x = Sep_Dist, y = Mean), size =2)+
        geom_line(data=gwcs_perf, aes(x = Sep_Dist, y = Mean), size = 1)+
...
plt_std <- ggplot() +
        geom_point(data=gwcs_perf, aes(x = Sep_Dist, y = StdeV), size =2)+
        geom_line(data=gwcs_perf, aes(x = Sep_Dist, y = StdeV), size = 1)+
...
plt_10kt <- ggplot() +
        geom_point(data=gwcs_perf, aes(x = Sep_Dist, y = gt_10kt_rate), size =2)+
        geom_line(data=gwcs_perf, aes(x = Sep_Dist, y = gt_10kt_rate), size = 1)+
...
plt_50nm <- ggplot() +
        geom_point(data=gwcs_perf, aes(x = Sep_Dist, y = gt_50nm_rate), size =2)+
        geom_line(data=gwcs_perf, aes(x = Sep_Dist, y = gt_50nm_rate), size = 1)+
...
# Performance by Runway
plt_mean <- ggplot() +
        geom_point(data=gwcs_perf_rw, aes(x = Sep_Dist, y = Mean,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size =2)+
        geom_line(data=gwcs_perf_rw, aes(x = Sep_Dist, y = Mean,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size = 1)+
...
plt_std <- ggplot() +
        geom_point(data=gwcs_perf_rw, aes(x = Sep_Dist, y = StdeV,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size =2)+
        geom_line(data=gwcs_perf_rw, aes(x = Sep_Dist, y = StdeV,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size = 1)+
...
plt_10kt <- ggplot() +
        geom_point(data=gwcs_perf_rw, aes(x = Sep_Dist, y = gt_10kt_rate,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size =2)+
        geom_line(data=gwcs_perf_rw, aes(x = Sep_Dist, y = gt_10kt_rate,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size = 1)+
...
plt_50nm <- ggplot() +
        geom_point(data=gwcs_perf_rw, aes(x = Sep_Dist, y = gt_50nm_rate,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size =2)+
        geom_line(data=gwcs_perf_rw, aes(x = Sep_Dist, y = gt_50nm_rate,colour=Landing_Runway_Dir, group = Landing_Runway_Dir), size = 1)+
...
plt_mean <- ggplot() +
        geom_point(data=gwcs_perf_surf, aes(x = Sep_Dist, y = Mean,colour=surface_wind_band, group = surface_wind_band), size =2)+
        geom_line(data=gwcs_perf_surf, aes(x = Sep_Dist, y = Mean,colour=surface_wind_band, group = surface_wind_band), size = 1)+
...
plt_std <- ggplot() +
        geom_point(data=gwcs_perf_surf, aes(x = Sep_Dist, y = StdeV,colour=surface_wind_band, group = surface_wind_band), size =2)+
        geom_line(data=gwcs_perf_surf, aes(x = Sep_Dist, y = StdeV,colour=surface_wind_band, group = surface_wind_band), size = 1)+
...
plt_10kt <- ggplot() +
        geom_point(data=gwcs_perf_surf, aes(x = Sep_Dist, y = gt_10kt_rate,colour=surface_wind_band, group = surface_wind_band), size =2)+
        geom_line(data=gwcs_perf_surf, aes(x = Sep_Dist, y = gt_10kt_rate,colour=surface_wind_band, group = surface_wind_band), size = 1)+
...
plt_50nm <- ggplot() +
        geom_point(data=gwcs_perf_surf, aes(x = Sep_Dist, y = gt_50nm_rate,colour=surface_wind_band, group = surface_wind_band), size =2)+
        geom_line(data=gwcs_perf_surf, aes(x = Sep_Dist, y = gt_50nm_rate,colour=surface_wind_band, group = surface_wind_band), size = 1)+
