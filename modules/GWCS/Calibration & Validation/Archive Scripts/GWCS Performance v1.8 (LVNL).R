...
PR_Sep_IAT <- perf_df %>% group_by(Sep_Dist, `IAT_Group`) %>% Performance_Table() 
...
PR_Ind_RG <- Indicator_Join(PR_Sep_RG) %>% group_by(Runway_Group) %>% Performance_Table_Indicator() %>% arrange(Runway_Group)
PR_Ind_MFO <- Indicator_Join(PR_Sep_MFO) %>% group_by(`max_forecast_offset`) %>% Performance_Table_Indicator() %>% arrange(`max_forecast_offset`)
...
PR_Arr_RG <- Performance_Table_Operational(PR_Ind_RG)
PR_Arr_MFO <- Performance_Table_Operational(PR_Ind_MFO)
...
fwrite(PR_Sep_IAT, file = file.path(out_data, "GWCS_Performance_Stats_Inter_Arrival.csv"))
