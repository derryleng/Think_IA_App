...
-- This script processes the UTMA Validation Database for eTBS Path Leg Tracking
-- Validation pre-processing for Heathrow.
...
-- Generate the FP data from radar data.
-- Not required for TBS logs - only where flight plan data NOT available.
--DELETE FROM tbl_Flight_Plan WHERE (FP_Date = @Log_Date OR @Log_Date IS null)
--EXEC usp_DL_Populate_tbl_Flight_Plan_From_Radar_Data @Log_Date
